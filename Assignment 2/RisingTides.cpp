...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> result = Grid(terrain.numRows(), terrain.numCols(), false);
    for (GridLocation location : sources) {
        if (terrain.get(location) <= height) {
            result.set(location, true);
    ...
    while (!queue.isEmpty()) {
        ...
        for (int k = 0; k < 4; ++k) {
            GridLocation tmp = GridLocation(location.row + dx[k], location.col + dy[k]);
            if (terrain.inBounds(tmp)) {
                if (!result.get(tmp) && terrain.get(tmp) <= height) {
                    result.set(tmp, true);
                    queue.enqueue(tmp);
                }
            }
