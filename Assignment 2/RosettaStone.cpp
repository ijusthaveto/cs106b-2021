...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    int length = str.size();
    if (length < kGramLength) {
        return {};
    }

    Map<string, double> map = Map<string, double>();
    int start = 0;
    while ((length- kGramLength) >= start) {
        string key = str.substr(start++, kGramLength);
        if (!map.containsKey(key)) {
            map.put(key, 1);
            continue;
        }
        double value = map.get(key) + 1;
        map.put(key, value);
    }
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    /* The input map doesn't contain nonzero values */
    int count = 0;
    ...
    if (count == input.size()) {
        error("Input map doesn't contain nonzero values!");
    }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    /* NumTokeep is bigger than the number of k-grams in the profile */
    if (numToKeep >= source.size()) {
        return source;
    }

    /* NumToKeep is negative */
    if (numToKeep < 0) {
...
double cosineSimilarityOf(const Map<string, double>& lhs, const Map<string, double>& rhs) {
    ...
    for (string key : lhs) {
        if (rhs.containsKey(key)) {
            result += lhs.get(key) * rhs.get(key);
        }
    }
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    if (corpora.size() == 0) {
        error("The set of corpora is empty!");
    }

    PriorityQueue<string> queue;
    for (Corpus language : corpora) {
        queue.enqueue(language.name, 2 - cosineSimilarityOf(textProfile, language.profile));
    }
