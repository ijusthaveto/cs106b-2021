...
bool checkTimeValid(const Set<Shift> & shifts, int maxHours);
bool checkShiftValid(const Set<Shift> & shifts, Shift & item);
int calcValue(const Set<Shift> & shifts);
Set<Shift> maxShifts(const Set<Shift> & oldSet, const Set<Shift> & newSet);
Set<Shift> highestValueScheduleRec(const Set<Shift> & ori, const Set<Shift> & cur, int maxHours);

Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    if (maxHours < 0) {
        error("Invalid maxHours!");
    }
...
bool checkTimeValid(const Set<Shift> & shifts, int maxHours) {
    ...
    for (const Shift & shift : shifts) {
        worktime += lengthOf(shift);
    }

    return worktime <= maxHours;
}
...
Set<Shift> maxShifts(const Set<Shift> & oldSet, const Set<Shift> & newSet) {
    int oldValue = calcValue(oldSet);
    int newValue = calcValue(newSet);
    return oldValue >= newValue ? oldSet : newSet;
}
...
Set<Shift> highestValueScheduleRec(const Set<Shift> & ori, const Set<Shift> & cur, int maxHours) {
    ...
    Shift tmp = ori.first();
    Set<Shift> remain = ori - tmp;
    Set<Shift> update = cur + tmp;

    if (checkTimeValid(update, maxHours) && checkShiftValid(cur, tmp)) {
        Set<Shift> oldSet = highestValueScheduleRec(remain, update, maxHours);
        Set<Shift> newSet = highestValueScheduleRec(remain, cur, maxHours);
        return maxShifts(oldSet, newSet);
    } else {
        return highestValueScheduleRec(remain, cur, maxHours);
    }
}
...
STUDENT_TEST("Test of my own.") {
    EXPECT_EQUAL(highestValueScheduleFor({}, 20), {});
}
