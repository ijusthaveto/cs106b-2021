...
bool disasterReadyRec(const Map<string, Set<string>> & roadNetwork,
                         int numCities,
                         Set<string> & supplyLocations,
                         Set<string> & remainderLocations) {
    /* Critical case */
    if (remainderLocations.isEmpty()) {
        return true;
    }

    /* If the number of the supplyLocations cannot cover all the city */
    if (!numCities && !remainderLocations.isEmpty()) {
        return false;
    }

    /* Recursive case */
    string city = remainderLocations.first();

    /* Select the city as the supplyLocation */
    supplyLocations.add(city);
    Set<string> possiblePlan = remainderLocations - city - roadNetwork.get(city);
    if (disasterReadyRec(roadNetwork, numCities - 1, supplyLocations, possiblePlan)) {
        return true;
    }

    supplyLocations.remove(city);

    /* Select the surrounding city as the supplyLocation */
    for (const string & neighbor : roadNetwork.get(city)) {
        supplyLocations.add(neighbor);
        possiblePlan = remainderLocations - neighbor - roadNetwork.get(neighbor);
        if (disasterReadyRec(roadNetwork, numCities - 1, supplyLocations, possiblePlan)) {
            return true;
        }
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
                            int numCities,
                            Set<string>& supplyLocations) {
    /* The number of the supplyLocations cannot be negative */
    if (numCities < 0) {
        error("Negative numCities");
    }

    /* If the roadNetwork is empty, we don't have to do anything */
    ...
    for (const string & city : roadNetwork) {
        remainderLocations.add(city);
    }

    return disasterReadyRec(roadNetwork, numCities, supplyLocations, remainderLocations);
}
