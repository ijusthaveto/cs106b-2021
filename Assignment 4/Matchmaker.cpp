...
bool hasPerfectMatching(const Map<string, Set<string>> &possibleLinks, Set<Pair> &matching)
{
    ...
    /* Recusion case */
    string current = possibleLinks.firstKey();
    auto friends = possibleLinks.get(current);

    /* Current doesn't have any friends */
    if (friends.isEmpty()) {
        return false;
    }

    for (const string & partner : friends) {
        auto remains = possibleLinks;
        remains.remove(partner);
        remains.remove(current);
        Pair newPair(current, partner);

        removePair(remains, current, partner);

        if (hasPerfectMatching(remains, matching)) {
